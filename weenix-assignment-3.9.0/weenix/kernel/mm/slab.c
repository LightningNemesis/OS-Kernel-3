...
#define rear_rz(cache, obj) (*(uintptr_t *)(((uintptr_t)(obj)) + (cache)->sa_objsize - sizeof(uintptr_t)))
...
#define next_obj(allocator, obj) \
        ((void *)(((uintptr_t)(obj)) + (allocator)->sa_objsize + sizeof(struct slab_bufctl)))
...
{
        return (((PAGE_SIZE << order) - sizeof(struct slab)) / (objsize + sizeof(struct slab_bufctl)));
}
...
_slab_waste(size_t objsize, int order)
{
         ...
         */
        return ((PAGE_SIZE << order) - _slab_size(objsize, _slab_nobjs(objsize, order)));
}
